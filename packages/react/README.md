...
Configure the worker with `setupDevToolWorker(...handlers)` from `@msw-dev-tool/core/browser`. The toolbar Mock Enable switch globally enables or bypasses HTTP and WebSocket mocks without losing individual settings. The UI also lets you change one HTTP handler, WebSocket endpoint, listener, or logical event branch independently. Listeners that declare logical message events show independent event branches beneath the listener.
