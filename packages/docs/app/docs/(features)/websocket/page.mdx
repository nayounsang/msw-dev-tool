...
The listener editor exposes both response values. It also provides an optional delay (default `0` ms) and a **Repeat** checkbox. When Repeat is enabled, set the interval in milliseconds and the total number of repetitions, including the first response. Use `Infinity` for an unbounded sequence; JSON and snapshots represent it as the string `"Infinity"`.
...
For a reproducible end-to-end check, start the example or another Node process with `setupDevToolServer()`, run `msw-dev-tool --pid <pid> ws-add-endpoint`, then `ws-add-listener` with the JSON below. Connect a real WebSocket client, record the delayed response count, update `customResponse` and Behavior, and query `ws-list` after each mutation. The same commands work through the Browser CLI with `--cdp-url` and `--target`; use Chrome DevTools MCP to inspect the selected tab and its WebSocket client. Delete the listener at the end to verify that pending responses stop.
