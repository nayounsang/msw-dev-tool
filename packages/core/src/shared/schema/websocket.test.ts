...
describe("websocket schema", () => {
  it("accepts string and regexp matchers in persisted endpoints", () => {
    expect(() =>
      webSocketEndpointsSchema.parse([
        {
          info: {
            id: "endpoint:string",
            kind: "websocket",
    ...
    expect(
      serializableWebSocketMatcherSchema.parse({
        kind: "regexp",
  ...
  it("accepts only WebSocket-compatible close arguments", () => {
    for (const code of [1000, 3000, 4999]) {
      expect(() =>
        webSocketBehaviorSchema.parse({ preset: "close", options: { code } }),
      ).not.toThrow();
    }
    for (const code of [999, 1001, 2999, 5000]) {
      expect(() => webSocketBehaviorSchema.parse({ preset: "close", options: { code } })).toThrow(
  ...
  it("validates listener scheduling and defaults old listeners", () => {
    expect(webSocketRepeatSchema.parse({ interval: 500, repetitions: "Infinity" })).toEqual({
      interval: 500,
      repetitions: "Infinity",
    });
    expect(webSocketRepeatSchema.parse({ interval: 0, repetitions: 3 })).toEqual({
      interval: 0,
      repetitions: 3,
    });
    expect(() => webSocketRepeatSchema.parse({ interval: 0, repetitions: "Infinity" })).toThrow(
      "Infinite WebSocket repetition requires a positive interval",
    );
