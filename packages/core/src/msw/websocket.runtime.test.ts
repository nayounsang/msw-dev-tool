...
describe("temporary WebSocket runtime", () => {
  it("applies temporary endpoint behavior and drops removed handlers from the runtime", async () => {
    ...
    expect(messages).toEqual([
      "Test message from MSW Dev Tool",
      "Test message from MSW Dev Tool",
      "Test message from MSW Dev Tool",
    ]);
    socket.close();
    ...
    store.getState().setWebSocketListenerBehavior(listenerId, {
      preset: "close",
      options: { code: 4001, reason: "temp-close" },
    });
...
describe("closeWebSocketConnections", () => {
  const setupCustomResponseHarness = async (path: string, eventTypes?: readonly string[]) => {
    ...
    const hook = Reflect.get(handler, WEBSOCKET_HANDLER_BIND) as {
  ...
  it("delays the first temporary default response", async () => {
    ...
    try {
      const { adapter, endpointId, listenerId, client, store } =
        await setupCustomResponseHarness("scheduled-default-delay");
  ...
  it("cancels a scheduled response when its response changes", async () => {
    ...
    try {
      const { adapter, endpointId, listenerId, client, store } =
        await setupCustomResponseHarness("cancel-scheduled");
      store.getState().setWebSocketListenerBehavior(listenerId, { preset: "default" });
      store.getState().setWebSocketListenerResponse(listenerId, {
        type: "send",
        dataType: "string",
  ...
  it("cancels all scheduled responses when the client disconnects", async () => {
    ...
    try {
      const { adapter, endpointId, listenerId, client, store } =
        await setupCustomResponseHarness("disconnect-scheduled");
      store.getState().setWebSocketListenerBehavior(listenerId, { preset: "default" });
      store.getState().setWebSocketListenerResponse(listenerId, {
        type: "send",
        dataType: "string",
  ...
  it("tracks concurrent delayed responses independently", async () => {
    ...
    try {
      const { adapter, endpointId, listenerId, client, store } =
        await setupCustomResponseHarness("concurrent-scheduled");
      store.getState().setWebSocketListenerBehavior(listenerId, { preset: "default" });
      store.getState().setWebSocketListenerResponse(listenerId, {
        type: "send",
        dataType: "string",
  ...
  it("stops an infinite response schedule when its schedule changes", async () => {
    ...
    try {
      const { adapter, endpointId, listenerId, client, store } =
        await setupCustomResponseHarness("infinite-scheduled");
      store.getState().setWebSocketListenerBehavior(listenerId, { preset: "default" });
      store.getState().setWebSocketListenerResponse(listenerId, {
        type: "send",
        dataType: "string",
  ...
  it("does not queue another repetition when the listener changes during send", async () => {
    ...
    try {
      const { adapter, endpointId, listenerId, client, store } =
        await setupCustomResponseHarness("change-during-send");
      store.getState().setWebSocketListenerBehavior(listenerId, { preset: "default" });
      store.getState().setWebSocketListenerResponse(listenerId, {
        type: "send",
        dataType: "string",
  ...
  it("falls back to the original listener when a default response is not configured", async () => {
    const { adapter, endpointId, listenerId, client, store } =
      await setupCustomResponseHarness("default-fallback");
    store.getState().setWebSocketListenerBehavior(listenerId, { preset: "default" });
    const original = vi.fn();
  ...
  it("delays a close response and closes only once", async () => {
    ...
    try {
      const { adapter, endpointId, listenerId, client, store } =
        await setupCustomResponseHarness("delayed-close");
  ...
  it("dispatches a configured custom string response", async () => {
    const { adapter, endpointId, listenerId, client, store } =
      await setupCustomResponseHarness("custom-string");
    store.getState().setWebSocketListenerCustomResponse(listenerId, {
      type: "send",
      dataType: "string",
      value: "hello",
  ...
  it("dispatches a configured ArrayBuffer response", async () => {
    const { adapter, endpointId, listenerId, client, store } =
      await setupCustomResponseHarness("custom-arraybuffer");
    store.getState().setWebSocketListenerCustomResponse(listenerId, {
      type: "send",
    ...
    adapter.dispatchWebSocketMessage(endpointId, client, new Event("message"), listenerId);
    expect(Array.from(new Uint8Array(client.send.mock.calls.at(-1)?.[0] as ArrayBuffer))).toEqual([
      104, 105,
    ]);
  });
  ...
  it("dispatches a configured Blob response", async () => {
    const { adapter, endpointId, listenerId, client, store } =
      await setupCustomResponseHarness("custom-blob");
    store.getState().setWebSocketListenerCustomResponse(listenerId, {
      type: "send",
      dataType: "Blob",
      value: "68 69",
  ...
  it("dispatches a configured close response", async () => {
    const { adapter, endpointId, listenerId, client, store } =
      await setupCustomResponseHarness("custom-close");
    store.getState().setWebSocketListenerCustomResponse(listenerId, {
      type: "close",
      code: 4001,
      reason: "Unauthorized",
