...
describe("browser-cli", () => {
  beforeEach(() => {
    ...
    cdp.listTargets.mockResolvedValue([
      pageTarget,
      { id: "worker", type: "service_worker", title: "Worker", url: "http://localhost:3000" },
    ]);
  });
  ...
  it("runs a shared command against the selected page bridge", async () => {
    const call = vi.fn().mockResolvedValueOnce({
      result: {
        value: { revision: 2, handlerCount: 1, handler: { id: "handler-a", behavior: "delay" } },
      },
    });
    const close = vi.fn();
    ...
    await expect(
      ...
      ok: true,
      revision: 2,
      handler: { id: "handler-a", behavior: "delay" },
      targetId: "page-a",
    });
