...
describe("listTargets", () => {
  it("returns the Chrome target list", async () => {
    vi.stubGlobal(
      ...
      vi
        ...
        .mockResolvedValue(
          new Response(
            JSON.stringify([{ id: "page", type: "page", title: "Page", url: "http://localhost" }]),
          ),
        ),
    );
  ...
  it("reports an unsuccessful Chrome target response", async () => {
    ...
    await expect(listTargets("http://localhost:9222")).rejects.toThrow(
      "Failed to list Chrome targets: 503 Unavailable",
    );
  });
  ...
  it("times out instead of leaving an agent command waiting for Chrome", async () => {
    ...
    const assertion = expect(request).rejects.toThrow(
      "Timed out while listing Chrome targets after 5ms",
    );
